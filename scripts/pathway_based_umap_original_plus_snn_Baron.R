...
for(set_id in c(3)){
  ...
  index<-read.table(paste0("Baron_",db, "/hp",set_id,"/stepfoward_final_path_index_cvdi3"), sep=",", header=F)
  ...
  counts.sub <- counts[geneNs,]
  dim(counts.sub)
  
  
  setwd(paste0("Baron_",db,"/hp",set_id,"/"))
  ...
  data.combined2.sub#10540 features across 2431 samples
  ...
  tiff(paste0("snn_stepwise_umap.tiff"), width=1000, height=500)
  print(dp)
  dev.off()
  ...
  pathindex = read.csv(paste0('Baron_',db,'/hp',set_id,'/stepfoward_final_path_index_cvdi3'), header=F)
  pathindex = unlist(pathindex)
  index2<-read.csv(paste0("Baron_",db,"/hp",set_id,"/original_model_gene_importance_cvdi4"), sep=",", header=T)
  ...
  data.combined2.sub=AddMetaData(object = data.combined2.sub, metadata = data.combined2@meta.data$orig.ident, col.name = "orig.ident")
  ...
  dp=DimPlot(data.combined3.sub, label=T, group.by="orig.ident")
  ...
  pathindex=read.csv(paste0('Baron_',db,'/hp',set_id,'/stepfoward_final_path_index_cvdi3'), header=F)
  pathindex=unlist(pathindex)
  index2<-read.csv(paste0("Baron_",db,"/hp",set_id,"/original_plus_pathways_model_gene_importance_cvdi4"), sep=",", header=T)
  # python based pathway index starting from 0
