...
def stepwise_forward(cvdi, target_pathways_fixed_p, target_pathways_testing_p, kf_p, X_train_p, y_train_p, Nnodes_p, Nlayers_p, optimizer_p, runN, epoch_p, batch_size_p, patience_p):
    #
    accuracies = []
    for test_pi in range(0, len(target_pathways_testing_p)):
        inner_cv_level2_accuracy = []
        for fi, (train_index, test_index) in enumerate(kf_p.split(X_train_p)):
            model2_0 = make_level2_model(len(target_pathways_fixed_p)+1, y_train_p[train_index,:].shape[1], Nnodes_p, Nlayers_p, optimizer_p)
            #
            ...
            for pi in target_pathways_fixed_p + [target_pathways_testing_p[test_pi]]:
                with open( runN + 
                        "/inner_cv_train_pathway_predictions/inner_cv_train_pathway_predictions_cvdi"+str(cvdi)+"_fi"+str(fi)+"_pi"+str(pi), 'rb') as file:
                    inner_cv_train_pathway_predictions.append(pickle.load(file))
                with open(runN + 
                        "/inner_cv_test_pathway_predictions/inner_cv_test_pathway_predictions_cvdi"+str(cvdi)+"_fi"+str(fi)+"_pi"+str(pi), 'rb') as file:
            ...
            #
            model2_0.fit(inner_cv_train_pathway_predictions, y_train_p[train_index,:], epochs=epoch_p, batch_size=batch_size_p, verbose=1, callbacks=[EarlyStoppingAtMinLoss(patience=patience_p)], validation_data=(inner_cv_test_pathway_predictions, y_train_p[test_index,:]))
            #
            loss, accuracy = model2_0.evaluate(inner_cv_test_pathway_predictions, y_train_p[test_index,:], verbose=0)
            inner_cv_level2_accuracy.append(accuracy)
        accuracies.append(sum(inner_cv_level2_accuracy) / len(inner_cv_level2_accuracy))
...
with open(runN+'/data/kf_cvdi'+str(cvdi), 'rb') as file:
    kf = pickle.load(file)
...
for tpi in range(0,len(testing_pi)):
    ...
    max_index, max_acc = stepwise_forward(cvdi=0, target_pathways_fixed_p=fixed_pi, target_pathways_testing_p=testing_pi, kf_p=kf, X_train_p=X_train, y_train_p=y_train, Nnodes_p=Nnodes, Nlayers_p=Nlayers, optimizer_p=optimizer, runN=runN, epoch_p=1, batch_size_p=batch_size_p, patience_p=patience_p)
    inner_cv_time_e =  datetime.now().replace(microsecond=0)
    time_stamp_f.write(f'{tpi}th pathway stepwise forwarad calculation in {cvdi}th outer cv-fold took {inner_cv_time_e - inner_cv_time_s}\n')
...
with open(runN+"/stepfoward_accuracy_cvdi"+str(cvdi), 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
    writer.writerows([past_acc])

with open(runN+"/stepfoward_final_path_index_cvdi"+str(cvdi), 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
...
for pi in fixed_pi:
    with open( runN + 
            "/outter_cv_train_pathway_predictions/outter_cv_train_pathway_predictions_cvdi0_pi"+str(pi), 'rb') as file:#this is predected on 0 test
        outter_cv_train_pathway_predictions.append(pickle.load(file))
    with open(runN + 
            "/outter_cv_train_pathway_predictions/outter_cv_train_pathway_predictions_cvdi1_pi"+str(pi), 'rb') as file:
...
model2_1.fit(outter_cv_test_pathway_predictions, y_train, epochs=epoch_p, batch_size=batch_size_p, verbose=1, callbacks=[EarlyStoppingAtMinLoss(patience=patience_p)], validation_data=(outter_cv_train_pathway_predictions, y_test))
#
loss, accuracy = model2_1.evaluate(outter_cv_train_pathway_predictions, y_test, verbose=0)



y_pred = model2_1.predict(outter_cv_train_pathway_predictions)
...
with open(runN+"/stepfoward_final_accuracy_cvdi"+str(cvdi), 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',')
...
trained_models=[]
for tpi in fixed_pi:
    trained_models.append(tf.keras.models.load_model(runN+"/trained_models/outter_model_cvdi"+str(cvdi)+"_pi"+str(tpi)+".keras"))
